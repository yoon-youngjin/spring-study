...
public class SsoAuthFilter implements Filter {
    ...
    public void doFilter(
        ...
        if (authToken.isPresent()) {
            ...
            Iterable<CookieProcess> all = logoutRepository.findAll();
            Iterator<CookieProcess> iterator = all.iterator();

            while (iterator.hasNext()){
                CookieProcess next = iterator.next();

                if (next.getId().equals(authToken.get())) {
