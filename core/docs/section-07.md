...
- 특징
  ...
  - 불변, 필수 의존관계에 사용 
    - 생성자는 단 한번만 호출되므로 불변
    - private final 이므로 생성자에 의해서 반드시 값이 set 되어야한다 -> 필수
...
스프링은 크게 2가지 라이프사이클로 나눠진다.
1. 빈 생성
2. 의존관계주입

단, 생성자 주입 OrderServiceImpl 객체를 생성하여 빈을 등록해야하므로 생성자를 호출하면서 의존관계주입도 같이 이뤄진다. -> 1, 2가 동시에
...
- 특징
  - **선택, 변경** 가능성이 있는 의존관계에 사용 -> @Autowired(required = false)를 통해 선택적인 주입도 가능하다. 이 경우에는 주입할 대상이 없더라도 괜찮다.
...
![image](https://user-images.githubusercontent.com/83503188/191790066-5278beac-c1cc-4403-a6e3-4d27bf183d9a.png)

해당 에러는 스프링 프레임워크를 띄우면서 나타난 오류이다.

```text
Caused by: org.springframework.beans.factory.support.BeanDefinitionOverrideException: Invalid bean definition with name 'memoryMemberRepository' defined in class path resource [dev/yoon/core/AutoAppConfig.class]: Cannot register bean definition [Root bean: class [null]; scope=; abstract=false; lazyInit=null; autowireMode=3; dependencyCheck=0; autowireCandidate=true; primary=false; factoryBeanName=autoAppConfig; factoryMethodName=memberRepository; initMethodName=null; destroyMethodName=(inferred); defined in class path resource [dev/yoon/core/AutoAppConfig.class]] for bean 'memoryMemberRepository': There is already [Generic bean: class [dev.yoon.core.member.MemoryMemberRepository]; scope=singleton; abstract=false; lazyInit=null; autowireMode=0; dependencyCheck=0; autowireCandidate=true; primary=false; factoryBeanName=null; factoryMethodName=null; initMethodName=null; destroyMethodName=null; defined in file [C:\Users\dudwl\WorkSpace\SSS\core\out\production\classes\dev\yoon\core\member\MemoryMemberRepository.class]] bound.
```

memoryMemberRepository 빈이 중복되었다는 문구이다. AuthAppConfig를 확인하면,
...
```
위와 같이 되어있어서 자동으로 등록된 memoryMemberRepository와 수동으로 등록된 memoryMemberRepository 사이에서 발생한 오류이다. 따라서 하나를 지워주면된다. 
...
- **Member**는 스프링 빈이 아니다.
- `setNoBean1()` 은 `@Autowired(required=false)` 이므로 메서드 호출 자체가 안된다.
...
프레임워크 없이 순수한 `OrderServiceImpl` 만 테스트하고싶은 경우 수정자 의존관계 주입을 이용한다면
...
![image](https://user-images.githubusercontent.com/83503188/191795257-4aaf4c16-4b99-4a54-a223-df5ee6c57318.png)
...
![image](https://user-images.githubusercontent.com/83503188/191797333-024518b5-5b7d-438f-b234-98f14b8f0204.png)
...
![image](https://user-images.githubusercontent.com/83503188/192080122-895e1197-07c8-4ec5-80e0-44adf89eb186.png)
...
`@Qualifier` 로 주입할 때 `@Qualifier("mainDiscountPolicy")` 를 못찾으면 어떻게 될까? 그러면 `mainDiscountPolicy`라는 이름의 스프링 빈을 추가로 찾는다.
...
public class AllBeanTest {
    ...
    public void findAllBean() throws Exception {
        // AutoAppConfig의 컴포넌트 스캔에 의해서 빈들이 등록되어서 주입이 가능해진다.
...
```
```text
policyMap = {fixDiscountPolicy=dev.yoon.core.discount.FixDiscountPolicy@793138bd, rateDiscountPolicy=dev.yoon.core.discount.RateDiscountPolicy@1951b871}
policies = [dev.yoon.core.discount.FixDiscountPolicy@793138bd, dev.yoon.core.discount.RateDiscountPolicy@1951b871]
```
...
- 예를 들어 DiscountPolicy를 타입으로 등록되는 빈을 한눈에 파악하고 싶을때

의존관계 자동 주입 - 조회한 빈이 모두 필요할 때, List, Map을 다시 보자. DiscountService 가 의존관계 자동 주입으로 `Map<String, DiscountPolicy>` 에 주입을 받는 상황을 생각해보자.
