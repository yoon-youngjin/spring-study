...
class UserControllerTest(
    ...
    fun `사용자는 인증 코드로 계정을 활성화 시킬 수 있다`() {
        ...
        // when, then
        mockMvc.perform(
            get("/api/users/${savedUserEntity.id}/verify")
                .queryParam("certificationCode", savedUserEntity.certificationCode)
        )
            .andExpect(status().isFound())

        userJpaRepository.findByIdOrNull(savedUserEntity.id)!!.status shouldBe UserStatus.ACTIVE
    }
    ...
    fun `사용자는 내 정보를 수정할 수 있다`() {
        ...
        mockMvc.perform(
            ...
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.nickname").value(userUpdate.nickname))
            .andExpect(jsonPath("$.address").value(userUpdate.address))
    }
}
